...
const validatePassword = (blur) => {
  ...
  if(password.length < minLength){
    ...
    if(blur) errors.value.password = `Password must contain lowercase letter`;
  } else if(!hasSpecialChar){
    if(blur) errors.value.password = `Password must contain special char`;
  } else {
...
const validateGender = (blur) => {
  ...
  if(!gender){
    if(blur) errors.value.gender = "Please select a  gender at least";
  } else {
